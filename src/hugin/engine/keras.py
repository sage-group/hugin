...
class KerasModel(RasterModel):
        ...
        **kwargs,
        ...
        if model_builder:
            if isinstance(model_builder, str):
                model_builder, model_builder_custom_options = import_model_builder(
                    model_builder
                )
            elif callable(model_builder):
                model_builder_custom_options = getattr(
                    model_builder, "custom_objects", {}
                )
            else:
    ...
    def find_model(self, checkpoint_epoch=None):
        ...
        checkpoint_directory = os.path.join(
            self.base_directory, self.checkpoint_subdirectory
        )
        if os.path.exists(checkpoint_directory):
            ...
            if checkpoint_epoch is not None:
                ...
                chkps = list(
                    filter(lambda x: int(x.split("-")[1]) == checkpoint_epoch, chkps)
                )
            if not chkps:
        ...
        if model_builder_options.get("input_shapes") is None:
            if self.input_shapes is None:
                if isinstance(train_data, tf.data.Dataset):
                    raise ValueError(
                        "Tensorflow Dataset based sources require"
                        "`input_shapes` specification"
                    )
    ...
    def fit_generator(
        self, train_data, validation_data=None, class_weights=None, sample_weights=None
    ):
            ...
            if validation_data is not None:
                if isinstance(validation_data, tf.keras.utils.Sequence):
                    ...
                    validation_steps_per_epoch = math.ceil(
                        len(validation_data) / self.batch_size
                    )
        ...
        if model_path and os.path.exists(model_path):
            ...
            if self.enable_multi_gpu:
                ...
                model = multi_gpu_model(
                    model, gpus=gpus, cpu_merge=cpu_merge, cpu_relocation=cpu_relocation
                )
