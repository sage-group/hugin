        ...
        if self.metrics:
            if self.gti_component in scene_data:
                ...
                for metric_name, metric_calculator in self.metrics.items():
                    my_metrics[metric_name] = metric_calculator(
                        original_result, gti.read()
                    )
            else:
                log.warning("Missing GTI data for GTI component: %s",
                            self.gti_component)
...
class GTICategoricalConverter(object):
    ...
    def __call__(self, entry):
        ...
        cat = tf.keras.utils.to_categorical(entry, self._num_classes)
        # print ("CAT:", cat.shape)
...
class TransformToCategorical(GTICategoricalConverter):
    def __init__(self, *args, **kw):
        GTICategoricalConverter.__init__(
            self,
        )
...
class RasterModel(HasTraits):
    ...
    def __init__(
        ...
        :param batch_size: Batch size used by `predict_scene_proba` when \
          sending data `predict`. Default: 1
...
class NullMerger(PredictionMerger):
    def __init__(self, *args, **kwargs):
        ...
        self.tile_array = np.zeros(
            (self.height, self.width, self.depth), dtype=self.dtype
        )
                ...
                try:
                    for input_name, data in tile[0].items():
                        if input_name not in self._multiple_input_scalers:
                            ...
                            out_data.reshape((out_data.shape + (1,)))
        ...
        if self._single_input:
            for id, scaler in enumerate(self._scalers):
                for i, mode in enumerate(["input", "gti_"]):
                    scaler_destination = os.path.join(
                        destination, mode + str(id) + ".pkl"
                    )
            ...
            for input_name, scaler in self._multiple_input_scalers.items():
                for i, mode in enumerate(["", "_gti"]):
                    scaler_destination = os.path.join(
                        destination, input_name + mode + ".pkl"
                    )
                    outs = pickle.dumps(scaler[i])
                    with open(scaler_destination, "wb") as f:
                        f.write(outs)
