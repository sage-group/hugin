...
class MultipleSceneModel:
    """
    This class is intended to be inherited by classes aimed to predict on \
      multiple scenes
    """
    ...
    def __init__(
        ...
        :param scene_id_filter: Regex for filtering scenes according to \
          their id (optional)
        :param randomize_training: Randomize list of traning data \
          between epochs
        """

        self.scene_id_filter = (
            None if not scene_id_filter else re.compile(scene_id_filter)
        )
    ...
    def predict_scenes_proba(self, scenes, predictor=None):
        ...
        for scene_id, scene_data in scenes:
            ...
            output = (
                scene_id,
                scene_data,
                predictor.predict_scene_proba((scene_id, scene_data)),
            )
            yield output
    ...
    def train_scenes(self, scenes, validation_scenes=None, trainer=None):
        ...
        if self.threaded:
            ...
            validation_data = ThreadedDataGenerator(
                validation_data, self.prefetch_queue_size
            )
...
class ArrayModelTrainer(ArrayModel):
    ...
    #
    # ToDo: the next method and the one from RasterSceneTrainer
    #  should go to a parent class
    def save(self, destination: str = None):
        destination = (
            destination if destination is not None else self.model.base_directory
        )
    ...
    def train(self, data_source: ZarrArrayLoader):
        ...
        if sample_weights is not None:
            log.warning(
                "Currently Tensorflow does not support specifying "
                "the wights and they are supposed to be inluced in the "
                "generator/sequence. IGNORING"
            )
            sample_weights = None
        ...
        if class_weights is None:
            ...
            raise NotImplementedError(
                "Unsupported representation for class weights: {}".format(
                    type(class_weights)
                )
            )
        self.model.fit_generator(
            training, validation, class_weights, sample_weights, **options
        )
...
class CoreScenePredictor(BaseSceneModel):
    def __init__(
        ...
        :param output_shape: Output shape of the prediction (Optional). \
          Inferred from input image size
        """
        BaseSceneModel.__init__(
            self,
        ...
        self.stride_size = stride_size if stride_size is not None \
            else self.window_size[0]

        if not mapping:
            raise TypeError(f"Missing `mapping` specification in {self.name}")
    ...
    def predict_scene_proba(self, scene, dataset_loader=None):
        ...
        tile_loader = (
            DatasetGenerator((scene,))
            if dataset_loader is None
            else dataset_loader.get_dataset_loader(scene)
        )
        tile_loader.reset()
        ...
        if len(output_mapping) == 1:  # Output is defined by GTI
            ...
            output_window_shape, output_stride_size = adapt_shape_and_stride(
                rio_raster,
        ...
        if tile_width != self.stride_size:
            xtiles = math.ceil((image_width - tile_width) /
                               float(self.stride_size) + 2)
        else:
            xtiles = math.ceil((image_width - tile_width) /
                               float(self.stride_size) + 1)

        if tile_height != self.stride_size:
            ytiles = math.ceil((image_height - tile_height) /
                               float(self.stride_size) + 2)
        else:
            ytiles = math.ceil((image_height - tile_height) /
                               float(self.stride_size) + 1)
        ...
        def prediction_generator(dgen):
            for data in dgen:
                ...
                prediction = self.predictor.predict(
                    in_arrays, batch_size=self.predictor.batch_size
                )
        ...
        while ytile < ytiles:
            ...
            while xtile < xtiles:
                ...
                tile_prediction = prediction.reshape(
                    tile_height, tile_width, prediction.shape[-1]
                )
                if merger is None:
                    merger = self.prediction_merger_class(
                        image_height,
                        image_width,
                        prediction.shape[-1],
                        prediction.dtype,
...
class BaseEnsembleScenePredictor(BaseSceneModel, MultipleSceneModel):
    def __init__(
        self, predictors, *args, name=None, resume=False, cache_file=None, **kwargs
    ):
        ...
        BaseSceneModel.__init__(
            ...
            post_processors=post_processors,
            metrics=metrics,
            gti_component=gti_component,
        )
        MultipleSceneModel.__init__(self, *args, **kwargs)
        self.predictors = predictors
        for predictor in predictors:
            predictor.metrics = self.metrics
            if predictor.gti_component is None:
                predictor.gti_component = self.gti_component
        self.resume = resume
        cache_file = (
            cache_file if cache_file is not None else NamedTemporaryFile("w+b").name
        )
    ...
    def predict_scenes_proba(self, scenes):
        ...
        for predictor_configuration in self.predictors:
            ...
            for scene_id, _, result in super(
                BaseEnsembleScenePredictor, self
            ).predict_scenes_proba(scenes, predictor):
                prediction, metrics = result
                dataset_name = "%s/%s" % (predictor.name, scene_id)
                if self.resume:
                    if dataset_name in self.cache.keys():
                        log.info(
                            "Scene %s already predicted using %s. Skipping!",
                            dataset_name,
                            predictor.name,
                        )
                        continue
...
class ArrayExporter(SceneExporter):
    ...
    def __init__(self, zarr_dataset, destination_array, *args, **kwargs):
        ...
        if self._format_options:
            self.destination_array = self.destination_array.format(
    ...
    def flow_prediction_from_array_loader(self, loader, predictor):
        ...
        for i in tqdm.tqdm(range(0, length)):
            ...
            if destination_array is None:
                ...
                destination_array = dataset.zeros(
            ...
            for pred_idx, dest_idx in enumerate(
                range(last_idx, last_idx + prediction_length)
            ):
...
class RasterIOSceneExporter(SceneExporter):
    ...
    def save_scene(
        self, scene_id, scene_data, prediction, destination=None, destination_file=None
    ):
        ...
        with rasterio.Env(**(self.rasterio_options)):
            ...
            profile.update(
                dtype=rasterio.dtypes.get_minimum_dtype(prediction),
                count=num_out_channels,
            )
...
class RasterScenePredictorMaxClass(RasterScenePredictor):
    ...
    def predict_scene_proba(self, *args, **kwargs):
        prediction, info = RasterScenePredictor.predict_scene_proba(self,
                                                                    *args,
                                                                    **kwargs)
