...
internal_logging_config = yaml.load(
    resource_stream(__name__, "/../_data/logging-config.yaml"), Loader=Loader
)
...
def main():
    parser = argparse.ArgumentParser(
        description="HuginEO -- Machine Learning for Earth Observation tool"
    )
    ...
    train_parser.add_argument(
        ...
        type=str,
        required=True,
        default=os.environ.get("HUGIN_CONFIG_URL", None),
        help="Path to config file",
    )
    train_parser.add_argument(
        "--input-dir", required=False, default=None, help=argparse.SUPPRESS
    )
