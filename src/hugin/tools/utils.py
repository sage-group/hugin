...
def to_categorical(*args, **kw):  # ToDo: maybe make generic
    return tf.keras.utils.to_categorical(*args, **kw)
...
class InTopK(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        return super(InTopK, self).update_state(matches, sample_weight=sample_weight)
...
def acc_scorer(y_true, y_pred):
    ...
    NOTE: This might be identical to Jaccard similarity score
...
def np_mean_iou(y_true, y_pred, labels):
    ...
    # Just in case we get a division by 0, ignore/hide the error and set
    # the value to 0
