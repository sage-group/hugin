...
class BaseLoader(object):
    ...
    def scan_datasets(self):
        ...
        if self._validation_source:
            self.update_datasets(
                input_source=self._validation_source,
                target_datasets=self._validation_datasets,
            )
        self.__update_split()
    ...
    def __update_split(self):
        ...
        if len(_validation_datasets) > 0:
            ...
            self._evaluation_list, self._train_list = tuple(
                _datasets[:num_evaluation]
            ), tuple(_datasets[num_evaluation:])
    ...
    def update_dataset(
        self, dataset=None, dataset_id=None, match_components={}, dataset_path=None
    ):
        if dataset is None:
            dataset = self._datasets
        ...
        if dataset_type in components:
            raise KeyError(
                f"Already registered: {dataset_id} {dataset_type} {dataset_path}"
            )
    ...
    def get_dataset_loaders(self):
        return self.build_dataset_loaders(
            self.get_training_datasets(), self.get_validation_datasets()
        )
    ...
    def get_dataset_loader(self, dataset, rasterio_env=None, _cache_data=None):
        ...
        return DatasetGenerator(
            (dataset,), rasterio_env=rasterio_env, _cache_data=_cache_data
        )

    def build_dataset_loaders(self, training_datasets, validation_datasets):
        training_loader = DatasetGenerator(
            training_datasets, rasterio_env=self.rasterio_env, _cache_data=self.cache_io
        )
        validation_loader = DatasetGenerator(
            validation_datasets,
            rasterio_env=self.rasterio_env,
...
class FileLoader(BaseLoader):
    ...
    def update_datasets(self, filter=None):
        ...
        if hasattr(self._input_source, "read"):
            ...
            close_file = False
        elif isinstance(self._input_source, str) and self._input_source.startswith(
            "gs://"
        ):
        ...
        for line in lines:
            ...
            dataset_id = self.update_dataset(
                match_components=match_components, dataset_path=dataset_path
            )
...
class FileSystemLoader(BaseLoader):
    ...
    def update_datasets(self, input_source=None, target_datasets=None, filter=None):
        if input_source is None:
            input_source = self.input_source
        if target_datasets is None:
            target_datasets = self.get_full_datasets()
        ...
        for directory_entry in os.walk(input_source, followlinks=True):
            ...
            for file_name in directory_members:
                ...
                dataset = self.get_dataset_by_id(dataset_id, dataset=target_datasets)
...
class FSSpecFilesystemLoader(BaseLoader):
    def __init__(
        ...
        **kw,
        ...
        if input_source is None:
            raise KeyError("Missing input_source")
        ...
        BaseLoader.__init__(
            ...
            *args,
            ...
            prepend_path=f"{url_spec.scheme}://",
            **kw,
        )

    def update_datasets(
        self, input_source=None, target_datasets=None, filter_dataset=None
    ):
        if input_source is None:
            input_source = self.input_source
        if target_datasets is None:
        ...
        for entry in all_files:
            ...
            dataset = self.get_dataset_by_id(dataset_id, dataset=target_datasets)
            if not filter_dataset(dataset_id, match_components, dataset):
                self.remove_dataset_by_id(dataset_id)
