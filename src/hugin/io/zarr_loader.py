...
class ArraySequence(Sequence):
    def __init__(
        ...
        if self.slice_timestamps is not None:
            assert isinstance(
                self.slice_timestamps, (tuple, list)
            ), "Timestamp slice should be list or tuple"
            assert len(self.slice_timestamps) == 3 or len(self.slice_timestamps) == 2
            if len(self.slice_timestamps) == 3:
                (
                    self.start_slice_idx,
                    self.end_slice_idx,
                    self.slice_step,
                ) = self.slice_timestamps
    ...
    def selected_indices(self):
        if self.__selected_indices is None:
            one_array = self.input_component_mapping[
                list(self.input_component_mapping.keys())[0]
            ]
    ...
    def selected_indices(self, v: Array):
        # Convert to NumPy array to prevent issues, memory impact should be minimal as
        # there should be a limited amount if indices
        data = (
            np.array(v) if v is not None else v
        )
        ...
        for flat_idx in indices[start_idx:end_idx]:
            for key, value in self.input_component_mapping.items():
                ...
                if self.slice_timestamps:
                    dask_data = dask_data[
                        self.start_slice_idx : self.end_slice_idx : self.slice_step, ...
                    ]
                data = np.array(dask_data)
                standardiser = self.standardisers.get(key, None)
                if standardiser is not None:
                    ...
                    for i in range(0, len(standardiser)):
                        ...
                        data[..., i] = channel_standardizer.transform(
                            data[..., i].reshape(-1, 1), copy=False
                        ).reshape(old_shape)
...
class ZarrArrayLoader(ArrayLoader):
    def __init__(
        ...
        if source is None:
            if "DATASOURCE_URL" not in os.environ:
                raise TypeError(
                    "Missing source specification. Should be specified directly or "
                    "as the `DATASOURCE_URL` environment variable"
                )
            else:
                source = os.environ["DATASOURCE_URL"]
                log.info(
                    "Using storage configuration from environment file "
                    f"`DATASOURCE_URL`: {source}"
                )
        up = urlparse(source)
        storage_options = {}
        if not up.scheme:
            self.source = source
            self.source = zarr.open_group(
                source, mode="r", storage_options=storage_options
            )
        else:
            ...
            if up.scheme == "s3":
                ...
                self.source = zarr.open_group(
                    source, mode="r", storage_options=storage_options, cache_attrs=False
                )
        ...
        if self.split_test_index_array_path:
            self.split_test_index_array = np.array(
                self.source[self.split_test_index_array_path]
            )
        if self.split_train_index_array_path:
            self.split_train_index_array = np.array(
                self.source[self.split_train_index_array_path]
            )

        if self.class_weights is not None:
            if isinstance(
                self.class_weights, str
            ):  # We received a path inside the `source`:
                ...
                raise NotImplementedError(
                    "No support for the specified type of class_weights"
                )
        ...
        if self.sample_weights is not None:
            if isinstance(
                ...
                raise NotImplementedError(
                    "No support for the specified type of sample_weights"
                )
        ...
        for input_name, input_path in inputs.items():
            ...
            if isinstance(input_path, dict):
                ...
                if standardizers is not None:
                    zarr_array = from_zarr(
                        source, standardizers, storage_options=self.storage_options
                    )
        ...
        for output_name, output_path in targets.items():
            ...
            if shape is not None:
                ...
                self.outputs[output_name] = self.outputs[output_name].reshape(
                    (outer_dimension,) + tuple(shape)
                )
    ...
    def get_training(self, batch_size: int) -> ArraySequence:
        ...
        return ArraySequence(
            ...
            self.outputs,
            batch_size,
            selected_indices=self.split_train_index_array,
            randomise=self.randomise,
            maximum_samples=self.maximum_training_samples,
            standardisers=self.input_standardizers,
            sample_weights=self.sample_weights,
            slice_timestamps=self.slice_timestamps,
            flat=self.flat,
    ...
    def get_validation(self, batch_size: int) -> ArraySequence:
        ...
        return ArraySequence(
            ...
            self.outputs,
            batch_size,
            selected_indices=self.split_test_index_array,
            randomise=self.randomise,
            maximum_samples=self.maximum_validation_samples,
            standardisers=self.input_standardizers,
            slice_timestamps=self.slice_timestamps,
            flat=self.flat,
            input_transformers=self.input_transformers,
...
def flatten_generator_data(data):
    ...
    for key in keys:
        ...
        value = value.reshape(
            value.shape[1:]
        )  ## Remove batch dimension. Works as long batch size is 1
        result.append(value)
    return tuple(result)
...
class ZarrArrayLoaderTFData(ZarrArrayLoader):
    ...
    def get_tfdataset(self, data):
        ...
        input_dtypes = [
            tf.dtypes.as_dtype(e.dtype) for e in flatten_generator_data(first_inputs)
        ]
        output_dtypes = [
            tf.dtypes.as_dtype(e.dtype) for e in flatten_generator_data(first_outputs)
        ]
        ...
        return tf.data.Dataset.from_generator(
            _standardise_data, output_types=output_types
        )
