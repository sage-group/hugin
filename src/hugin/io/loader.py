...
from ..tools.utils import to_categorical
...
class MulticlassRemappingConverter(CategoricalConverter):
    def __init__(self, *args, mapping={}, **kw):
        if isinstance(mapping, dict):
            ...
            raise ValueError(
                "Unsupported format for mapping." "Should be list, tuple or dictionary"
            )
...
def adapt_shape_and_stride(scene, base_scene, shape, stride, offset="center"):
    ...
    # The following snipped is a workaround for intermitent issues regarding \
    # coregistration between input scene and GTI, even if they are of the same \
    # resolution
    # Should be fixed by getting rid of cartesian indexing and using only \
...
class TileGenerator(object):
    def __init__(
        ...
        """
        @shape: specify the shape of the window/tile. None means the window covers the\
           whole image
    ...
    def __len__(self):
        ...
        if tile_width != self._stride:
            num_horiz = math.ceil((scene_width - tile_width) /
                                  float(self._stride) + 2)
        else:
            num_horiz = math.ceil((scene_width - tile_width) /
                                  float(self._stride) + 1)
        if tile_height != self._stride:
            num_vert = math.ceil((scene_height - tile_height) /
                                 float(self._stride) + 2)
        else:
            num_vert = math.ceil((scene_height - tile_height) /
                                 float(self._stride) + 1)
    ...
    def _generate_tiles_for_mapping(self,
                                    dataset,
                                    mapping, target_shape, target_stride):
        ...
        while ytile < ytiles:
            ...
            while xtile < xtiles:
                ...
                for map_entry in augmented_mapping:
                    ...
                    preproc_callbacks = map_entry.get("preprocessing", [])
                    ...
                    if transform_expression is not None:
                        raise NotImplementedError(
                            "Snuggs expressions are currently not implemented"
                        )

                    all_callbacks = mapping_level_preprocessing + preproc_callbacks
                    for callback in all_callbacks:
                        band = callback(band)

                    if buffer is None:
                        buffer = np.zeros(
                            (len(augmented_mapping),) + band.shape, dtype=band.dtype
                        )

                    if buffer.dtype != band.dtype:
                        buffer = buffer.astype(
                            np.find_common_type([buffer.dtype, band.dtype], [])
                        )
                    buffer[count - 1] = band
    ...
    def generate_tiles_for_dataset(self):
        ...
        for mapping_name, mapping in input_mapping.items():
            ...
            input_generators[mapping_name] = self._generate_tiles_for_mapping(
                self._scene, mapping, target_shape, target_stride
            )

        for mapping_name, mapping in output_mapping.items():
            target_shape, target_stride = mapping["window_shape"], mapping["stride"]
            output_generators[mapping_name] = self._generate_tiles_for_mapping(
                self._scene, mapping, target_shape, target_stride
            )
...
class DataGenerator(object):
    def __init__(
        ...
        if len(input_mapping.keys()) == 1:  # One mapping. Should be primary by default
            input_mapping[list(input_mapping.keys())[0]]["primary"] = True

        primary_mapping = [
            input_mapping[m]
            for m in input_mapping
            if input_mapping[m].get("primary", False)
        ]
        ...
        if default_window_size is not None:
            ...
            log.warning(f"No default `window_size`. Using computed one: {window_size}")
        else:
            window_size = None
        ...
        for entry in self._mapping:
            if entry is None:  # Probably missing GTI
                continue
            for mapping_type, mapping_value in entry.items():
                if "window_shape" not in mapping_value or "stride" not in mapping_value:
                    ...
                    entry_shape, entry_stride = adapt_shape_and_stride(
                        current_scene,
                        self.primary_scene,
    ...
    def mapping_sizes(self):
        ...
        for input_name, input_value in input_mapping.items():
            input_sizes[input_name] = tuple(input_value["window_shape"]) + (
                len(input_value["channels"]),
            )
        if output_mapping is not None:
            for output_name, output_value in output_mapping.items():
                output_sizes[output_name] = tuple(output_value["window_shape"]) + (
                    len(output_value["channels"]),
                )
    ...
    def _convert_mapping(self, endpoint, mapping, primary):
        ...
        new_mapping[mapping_name] = {"primary": primary, "channels": mapping}
        if (
            hasattr(self, "primary_window_shape")
            and self.primary_window_shape is not None
        ):
        ...
        for scene in dataset_loader:
            ...
            for entry in tile_generator:
                ...
                for callback in callbacks:
                    input_patches, output_patches = callback(
                        input_patches, output_patches
                    )

                for in_patch_name, in_patch_value in input_patches.items():
                    if in_patch_name not in input_data:
                        input_data[in_patch_name] = np.zeros(
                            (self._batch_size,) + in_patch_value.shape,
                            dtype=in_patch_value.dtype,
                        )
                ...
                for out_patch_name, out_patch_value in output_patches.items():
                    ...
                    if out_patch_name not in output_data:
                        output_data[out_patch_name] = np.zeros(
                            (self._batch_size,) + new_path_value.shape,
                            dtype=new_path_value.dtype,
                        )
                ...
                if count == self._batch_size:
                    ...
                    result = (
                        self._flaten_simple_input(in_arrays),
                        self._flaten_simple_input(out_arrays),
                    )
