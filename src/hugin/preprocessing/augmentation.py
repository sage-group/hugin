...
class Augmentation(object):
            ...
            if isinstance(gti, dict):
                ...
                if aug_gti:
                    for k, v in gti.items():
                        gti_aug = seq_det.augment_image(v) if self.augment_outputs \
                            else v
    ...
    def _sequencer(self):
        ...
        if "CoarseDropout" in op:
            operators.append(
                iaa.Sometimes(
                    op["CourseDropout"]["prop"],
                    iaa.CoarseDropout(
                        op["CourseDropout"]["p"], op["CourseDropout"]["size_percent"]
                    ),
                    deterministic=True,
                )
            )
        ...
        for k, v in op.items():
            ...
            if isinstance(v, float):
                try:
                    ...
                    log.error(
                        "Simple operator {} failed to instantiate with {} and {}"
                        .format(
                            k, type(inst), inst.args
                        )
                    )
                    sys.exit()
            elif "prop" in v:
                ...
                try:
                    ...
                    log.error(
                        ...
                        "{} failed to instantiate with {} and {}".format(
                            k, type(inst), inst.args
                        )
                    )
                    sys.exit()
                ...
                try:
                    ...
                    log.error(
                        "Operator {} failed to instantiate with {} and {}".format(
                            k, type(inst), inst.args
                        )
                    )
                    sys.exit()
    ...
    def legacy_aug(
        ...
        def _rotate90(img, ground_truth, u=0.5, v=1.0):
            if v < u:
                ...
                ground_truth_m = cv2.getRotationMatrix2D(
                    (ground_truth_cols / 2, ground_truth_rows / 2), angle, 1
                )
                ground_truth = cv2.warpAffine(
                    ground_truth, ground_truth_m, (ground_truth_cols, ground_truth_rows)
                )
            return img, ground_truth
        ...
        def _rotate(img, ground_truth, angle=45, u=0.5, v=1.0):
            if v < u:
                ...
                ground_truth = cv2.warpAffine(
                    ground_truth, ground_truth_m, (ground_truth_cols,
                                                   ground_truth_rows)
                )
        ...
        def _shift(img, ground_truth, rcol=0.1, rrow=0.1, u=0.5, v=1.0):
            if v < u:
                ...
                ground_truth_rows, ground_truth_cols = ground_truth.shape
                ground_truth_m = np.float32(
                    [[1, 0, ground_truth_rows * rrow], [0, 1, ground_truth_cols * rcol]]
                )
                ground_truth = cv2.warpAffine(
                    ground_truth, ground_truth_m, (ground_truth_cols, ground_truth_rows)
                )
        ...
        def _zoom(img, ground_truth, zoom_rg=(0.1, 0.1), u=0.5, v=1.0):
            ...
            if v < u:
                img = prep.random_zoom(
                    img, zoom_range=zoom_rg, row_axis=0, col_axis=1, channel_axis=2
                )
                ground_truth = prep.random_zoom(
                    ground_truth,
                    zoom_range=zoom_rg,
                    row_axis=0,
                    col_axis=1,
        ...
        if zoom:
            X, y = _zoom(
                X, y, zoom_rg=zoom["zoom_rg"], u=zoom["prob"], v=np.random.random()
            )

        if laplace:
            X, y = _laplace(
                X, y, ksize=laplace["ksize"], u=laplace["prob"], v=np.random.random()
            )
